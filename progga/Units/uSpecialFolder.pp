...
// Позволяет получить специальную папку по CSIDL-значению (Windows XP и ранее)
// или FOLDERID (Windows Vista и новее)
function GetSpecialFolderLocation(const Folder: Integer; const FolderNew: TGUID): String;

implementation

uses
  Classes, windows, SysUtils, ActiveX, ShlObj;
...
begin
  ...
  begin
    Pointer(SHGetKnownFolderPath) := GetProcAddress(GetModuleHandle('Shell32.dll'), 'SHGetKnownFolderPath');
  ...
  begin
    Pointer(SHGetFolderPath) := GetProcAddress(GetModuleHandle('Shell32.dll'), 'SHGetFolderPathW');
